// zolkrat.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
// Function to convert a wide string to a multi-byte string
char* ConvertWideToMultiByte(PWSTR wideStr) {
    int size_needed = WideCharToMultiByte(CP_UTF8, 0, wideStr, -1, NULL, 0, NULL, NULL);
    char* multiByteStr = (char*)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, size_needed);
    if (multiByteStr) {
        WideCharToMultiByte(CP_UTF8, 0, wideStr, -1, multiByteStr, size_needed, NULL, NULL);
    }
    return multiByteStr;
}

// Function to parse the command line
void ParseCommandLine(PWSTR cmdLine, int& argc, char**& argv) {
    argc = 0;
    argv = nullptr;
    ...
    argv = (char**)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, argc * sizeof(char*));
    if (!argv) return;
    ...
    while (*cmd) {
        if (*cmd == L'"') {
            ...
            if (argStart) {
                argv[index] = ConvertWideToMultiByte(argStart);
                index++;
                argStart = nullptr;
            }
            *cmd = L'\0';
...
extern "C" __declspec(thread) DWORD _tls_index = TLS_OUT_OF_INDEXES;


extern "C" int open(const char* path, int oflag, int pmode) {
    ...
    return (int)hFile; // You might need to adjust the return value depending on how you handle file descriptors
}
...
/*
int __cdecl mainCRTStartup(PPEB pPeb)
{
    char text[64]; 
    ...
    PRTL_USER_PROCESS_PARAMETERS pProcParams = ((PPEB)pPeb)->ProcessParameters;
    int argc;
    char** argv;
    ParseCommandLine(pProcParams->CommandLine.Buffer, argc, argv);
